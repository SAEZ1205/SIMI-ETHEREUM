...
export async function signOperatorRequest(request) {
  const provider = new BrowserProvider(ethereum());
  const signer = await provider.getSigner();
  const operator = await signer.getAddress();
  const holder = request.holderAddress || operator;
  ...
  const value = {
    requestId, subscriberHash, holder,
    nonce: Number(request.nonce || 1),
    createdAt: Number(request.createdAt),
    expiresAt: Number(request.expiresAt),
    policyVersion: Number(request.policyVersion || 2),
  };
  const signature = await signer.signTypedData(domain, types, value);
  return { signature, signer: operator, payload: value, domain };
}
...
export async function signHolderDecision(request, action) {
  ...
  const holder = request.holderAddress || signerAddress;
  if (holder.toLowerCase() !== signerAddress.toLowerCase()) throw new Error('HOLDER_ACCOUNT_MISMATCH');
  ...
  const signature = await signer.signTypedData(domain, types, value);
  return { signature, signer: signerAddress, payload: value, domain };
}
